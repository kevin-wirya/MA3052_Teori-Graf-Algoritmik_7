...
public class ConnectedRouteVisual extends JFrame {
    private GraphPanel graphPanel;
    
    public ConnectedRouteVisual(ArrayList<ArrayList<Integer>> adj, ArrayList<Integer> dfsTraversal, boolean isConnected) {
        setTitle("Visualisasi Keterhubungan Lintasan a dan b - " + (isConnected ? "TERHUBUNG (Ya)" : "TIDAK TERHUBUNG (Tidak)"));
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setSize(1000, 700);
        setLocationRelativeTo(null);
        setResizable(true);
        
        graphPanel = new GraphPanel(adj, dfsTraversal, isConnected);
        add(graphPanel);
        ...
        String resultText = "Hasil: " + (isConnected ? "LINTASAN TERHUBUNG (Ya)" : "LINTASAN TIDAK TERHUBUNG (Tidak)");
        JLabel resultLabel = new JLabel(resultText);
        resultLabel.setForeground(isConnected ? new Color(0, 255, 0) : new Color(255, 0, 0));
        resultLabel.setFont(new Font("Arial", Font.BOLD, 16));
        resultLabel.setAlignmentX(Component.CENTER_ALIGNMENT);
        
        JLabel traversalLabel = new JLabel("DFS Traversal Order: " + dfsTraversal.toString());
...
class GraphPanel extends JPanel {
    ...
    public GraphPanel(ArrayList<ArrayList<Integer>> adj, ArrayList<Integer> dfsTraversal, boolean isConnected) {
        this.adj = adj;
        ...
        for (int step = 1; step < dfsTraversal.size(); step++) {
            int currentNode = dfsTraversal.get(step);
            for (int j = step - 1; j >= 0; j--) {
                int candidate = dfsTraversal.get(j);
    ...
    private void startAnimation() {
        ...
        animationThread = new Thread(() -> {
            for (int step = 0; step < dfsTraversal.size(); step++) {
                if (stopAnimation) {
                    isAnimating = false;
                    return;
                }
    ...
    protected void paintComponent(Graphics g) {
        ...
        g2d.setColor(Color.BLACK);
        g2d.drawString("Visited Node", legendX + 30, legendY + 30);
        
        // Status
        g2d.setColor(Color.BLACK);
        g2d.setFont(new Font("Arial", Font.BOLD, 13));
        if (isAnimating) g2d.drawString("Status: Step " + currentStep + "/" + dfsTraversal.size(), legendX, legendY - 20);
        else if (visitedNodes.isEmpty()) g2d.drawString("Status: Ready", legendX, legendY - 20);
        else {
            String status = isConnected ? "Status: TERHUBUNG (Ya)" : "Status: TIDAK TERHUBUNG (Tidak)";
            g2d.setColor(isConnected ? new Color(0, 150, 0) : new Color(200, 0, 0));
            g2d.drawString(status, legendX, legendY - 20);
        }
    }
}
