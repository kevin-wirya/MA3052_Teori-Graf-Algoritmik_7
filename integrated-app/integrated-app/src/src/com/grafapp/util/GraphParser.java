...
public class GraphParser {
    ...
    public static Graph parseFromFile(String filePath) throws IOException {
        return parseEdgeList(readFileText(filePath), false);
    }

    /** Baca teks mentah dari resource classpath */
    public static String readFileText(String filePath) throws IOException {
        try (InputStream inputStream = openResourceStream(filePath)) {
            if (inputStream == null) {
    ...
    private static InputStream openResourceStream(String path) {
        ...
        InputStream inputStream = GraphParser.class.getResourceAsStream(normalized);
        if (inputStream == null && normalized.startsWith("/")) {
            inputStream = GraphParser.class.getClassLoader().getResourceAsStream(normalized.substring(1));
        }
        return inputStream;
    }
