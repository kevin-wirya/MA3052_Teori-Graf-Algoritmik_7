...
- **14 Algoritma Built-in**: DFS, BFS, Connectivity Check, Connected Components, Largest Component, Path Finder (A -> B), Shortest Path (Dijkstra), Traveling Salesman (Held-Karp Exact), Minimum Spanning Tree (Kruskal), Minimum Spanning Tree (Prim), Bipartite Check, Cycle Detection, Graph Diameter, Graph Girth
